...
public class UserInfoController extends AdminBaseController {
	...
	@RequestMapping(value = "modifyPwdForm")
	public String modifyPwdForm(UserInfo userInfo, Model model) {
		model.addAttribute("userInfo", userInfo);
		return "modules/user/userPasswordForm";
	}
	...
	public @ResponseBody ResultMessage modifyPwd(UserInfo userInfo,
			String oldPassword, String newPassword, Model model) {
		if (StringUtils.isBlank(userInfo.getNewPassword())) {
			newPassword = StringUtils.substring(userInfo.getMobile(), 0, 6);
			userInfo.setNewPassword(newPassword);
			userInfoCommonService.updatePassword(userInfo,
