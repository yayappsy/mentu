...
@EnableWebMvc
@EnableSwagger2
@ComponentScan(basePackages = { "com.weimhc.api.modules",
		"com.weimhc.api.report", "com.weimhc.api.core" })
public class SwaggerConfig {
	...
	private List<ApiKey> apiKeys() {
		List<ApiKey> apiKeys = newArrayList(
				new ApiKey("timestamp", "timestamp", "header"),
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title(appName).description(appDesc)
				...
				.license(license).licenseUrl(licenseUrl).version(appVersion)
				.build();
	}
	...
	SecurityScheme oauth() {
		return new OAuthBuilder().name("petstore_auth").grantTypes(grantTypes())
				.scopes(scopes()).build();
	}

	List<AuthorizationScope> scopes() {
		return newArrayList(new AuthorizationScope[] {
	...
	List<GrantType> grantTypes() {
		GrantType authorizationCodeGrant = new AuthorizationCodeGrant(
				new TokenRequestEndpoint(authorizationUrl, authClientId,
						authClientSecret),
	...
	List<SecurityReference> defaultAuth() {
		...
		SecurityReference securityReference = SecurityReference.builder()
				.reference("petstore_auth").scopes(petstoreAuthScopes).build();
		return newArrayList(new SecurityReference("api_key", apiKeyScopes),
				securityReference);
	}

	SecurityConfiguration security() {
		return new SecurityConfiguration("test-app-client-id", "test-app-realm",
				"test-app", "apiKey", null, null, null, null);
	}
}