...
<body>
	...
	<c:choose>
		<c:when test="${empty genTable.name}">
			<form:form id="genTableForm" modelAttribute="genTable" action="${ctx}/gen/genTable/form" method="post" class="form-horizontal">
				<form:hidden path="id"/>
		...
		<c:otherwise>
			<form:form id="genTableForm" modelAttribute="genTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				<form:hidden path="id"/>
				<sys:message content="${message}"/>
				<fieldset>
					<legend>基本信息</legend>
						...
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							...
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<input type="text" name="columnList[${vs.index}].code" value="${column.code}" maxlength="200" class="required input-small"/>
									</td>
									<td>
