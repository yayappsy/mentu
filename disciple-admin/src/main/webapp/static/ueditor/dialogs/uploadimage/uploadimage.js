...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader = _this.uploader = WebUploader.create({
                pick: {
                    ...
                    //是否可以选择多个图片上传
                    multiple: editor.options.selectMultiple,
                },
                accept: {
                    title: 'Images',
            ...
            //是否可以选择多个图片上传,不允许则不能添加继续上传按钮
            uploader.addButton(editor.options.selectMultiple?{
                id: '#filePickerBtn',
        ...
        getInsertList: function () {
            var i, data, list = [],
                align = getAlign(),
                prefix = editor.getOpt('imageUrlPrefix');/*不使用该参数，有程序在后台控制**/
            for (i = 0; i < this.imageList.length; i++) {
                data = this.imageList[i];
                list.push({
                    src:  data.url,
                    _src: data.filePath,/*修改url为filepath，**/
                    thumbnail: data.thumbnail,/*修改url为filepath，**/
                    thumbPath: data.thumbPath,/*修改url为filepath，**/
    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on(this.container, 'click', function (e) {
                var target = e.target || e.srcElement,
                    ...
                    list = onlineImage.getInsertList();
                 /*判断是否可以多选，如果不可以则给出提示*/
                if(!editor.options.selectMultiple && list.length >= 1){
                	alert("不允许选择多张图片！");
                	return false;
                }
                ...
                if (li.tagName.toLowerCase() == 'li') {
                    if (domUtils.hasClass(li, 'selected')) {
                        ...
                        /*移除选择顺序*/
                        _this.deleteSelectedIndex(li);                        
                    } else {
                        domUtils.addClass(li, 'selected');
                         /*增加选择顺序*/
                        _this.addSelectedIndex(li);
                    }
                }
            });
        },
        ...
        addFile: function(file){
        	var _this = this,urlPrefix = editor.getOpt('imageManagerUrlPrefix'),
        	     delUrl = editor.getActionUrl(editor.getOpt('imageDelActionName')),
        	     isJsonp = utils.isCrossDomainUrl(delUrl);
        	var $li = $('<li id="'+file.id+'">' +
                    '<span class="icon"></span>' +
                    '</li>'),
                $btns = $('<div class="file-panel">' +
                        '<span class="cancel">' + lang.uploadDelete + '</span></div>').appendTo($li);
                img = document.createElement('img'),
                $img = $(img).prependTo($li);//保证是第一个元素，供getInsertList方法使用
            ...
            $img.attr("width","113");
            $img.attr('src', file.url + (file.url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
            //$img.attr('_src', file.filePath);
            $img.attr('_src', file.filePath);
            $img.attr("data-img-index",i);
            $img.attr("imgWidth",file.imgWidth);
            $img.attr("imgHeight",file.imgWidth);
            $img.attr("ext",file.ext);
            $img.attr("thumbnail",file.thumbnail);/*缩略图*/
            $img.attr("thumbPath",file.thumbPath);/*缩略图文件位置*/


            $li.insertBefore(this.clearFloat);
        ...
        getInsertList: function () {
            var i, lis = this.list.children, list = [],selectList=[], align = getAlign(),imgList=[];
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    var img = lis[i].firstChild,
                        src = img.getAttribute('src'),
                        _src = img.getAttribute('_src'),
                        index = parseInt(lis[i].getAttribute("data-img-selected-index"));
                    list[index] = {
                        src: src,
                        _src: _src,
                        thumbnail:img.getAttribute("thumbnail"),
                        thumbPath:img.getAttribute("thumbPath"),
                        alt: _src.substr(_src.lastIndexOf('/') + 1),
                        floatStyle: align,
                        ext:img.getAttribute("ext"),
                        imgWidth:img.getAttribute("imgWidth"),
                        imgHeight:img.getAttribute("imgHeight")
                    };
                }

            }
            this.clearNullAttr(list);
            return list;
        },
        /*清除空选项*/
        clearNullAttr: function(array){
        ...
        /* 增加选择图片顺序 */
  	    addSelectedIndex: function(obj){
  	  	    obj.setAttribute("data-img-selected-index",this.selectIndex);
  	  	    this.selectIndex +=1;
  	  	},
  	    /* 移除选择图片顺序 */
  	  	deleteSelectedIndex: function(obj){
  	  	   obj.removeAttribute("data-img-selected-index");
  	    },        
    };
