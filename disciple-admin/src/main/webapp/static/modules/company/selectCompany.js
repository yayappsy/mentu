$(document).ready(function() {
    ...
    $('#btnCompanySelectRemove').on("click",function() {
        $("#companyId").val("");
        $("#companyName").val("");
    });
    $('#btnCompanySelect').on("click",function() {
        layer.open({
            ...
            //开启最大化最小化按钮
            area: ['800px', '600px'],
            content: ctx+'/company/company/list?searchType=selectLink',
            btn: ['确定', '关闭'],
            yes: function(index, layero) {
                ...
                if (selectData.length > 0) {
                    $("#companyId").val(selectData[0].id);
                    $("#companyName").val(selectData[0].name);
                }
                layer.close(index);
