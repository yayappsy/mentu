 ...
 * 
 */
package com.weimhc.modules.gen.util;
...
import com.weimhc.modules.gen.entity.GenCategory;
import com.weimhc.modules.gen.entity.GenConfig;
...
public class GenUtils {
	...
	private static final String TEMPLATES_GEN_PATH = "templates.gen.path";
	private static final String TEMPLATES_GEN_CONFING_NAME = "templates.gen.config.name";
	...
	public static void initColumnField(GenTable genTable) {
		for (GenTableColumn column : genTable.getColumnList()) {
			...
			if (StringUtils.isNotBlank(column.getId())) {
				// 设置国际化字段
				column.setCode(
						genTable.getClassName() + "." + column.getJavaField());
				continue;
			}
			...
			if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "CHAR")
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(),
							"VARCHAR")
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(),
							"NARCHAR")) {
				column.setJavaType("String");
			} else if (StringUtils.startsWithIgnoreCase(column.getJdbcType(),
					"DATETIME")
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(),
							"DATE")
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(),
				...
				column.setShowType("dateselect");
					...
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(),
							"NUMBER")) {
				// 如果是浮点型
				String[] ss = StringUtils.split(StringUtils
						.substringBetween(column.getJdbcType(), "(", ")"), ",");
				if (ss != null && ss.length == 2
						&& Integer.parseInt(ss[1]) > 0) {
					column.setJavaType("Double");
				}
				// 如果是整形
				else if (ss != null && ss.length == 1
						&& Integer.parseInt(ss[0]) <= 10) {
			...
			// 是否是主键
			column.setIsPk(genTable.getPkList().contains(column.getName()) ? "1"
					: "0");
			...
			if (StringUtils.startsWithIgnoreCase(column.getName(), "user_id")) {
				...
				column.setJavaField(
						column.getJavaField().replaceAll("Id", ".id|name"));
				column.setShowType("userselect");
			}
			// 区域
			else if (StringUtils.startsWithIgnoreCase(column.getName(),
					"region_id")) {
				column.setJavaType(Region.class.getName());
				column.setJavaField(
						column.getJavaField().replaceAll("Id", ".id|name"));
				column.setShowType("regionselect");
			}
			// 创建者、更新者
			else if (StringUtils.startsWithIgnoreCase(column.getName(),
					"create_by")
			...
			// 父级ID
			else if (StringUtils.equalsIgnoreCase(column.getName(),
					"parent_id")) {
	...
	public static String getTemplatePath() {
		return SystemPath.getClassPath() + File.separator
				+ Global.getConfig(TEMPLATES_GEN_PATH);
	}
	...
	public static <T> T fileToObject(String fileName, Class<?> clazz) {
		try {
			String pathName = Global.getConfig(TEMPLATES_GEN_PATH) + fileName;
	...
	public static GenConfig getConfig() {
		return fileToObject(Global.getConfig(TEMPLATES_GEN_CONFING_NAME),
				GenConfig.class);
	}
	...
	public static List<GenTemplate> getTemplateList(GenConfig config,
			String category, boolean isChildTable) {
					...
					if (list != null) {
						for (String s : list) {
							if (StringUtils.startsWith(s,
								...
								GenTemplate template = fileToObject(s,
										GenTemplate.class);
								if (template != null) {
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme) {
		...
		model.put("packageName",
				StringUtils.lowerCase(genScheme.getPackageName()));
		model.put("lastPackageName", StringUtils
				.substringAfterLast((String) model.get("packageName"), "."));
		model.put("beforeLastPackageName", StringUtils
				.substringBeforeLast((String) model.get("packageName"), "."));
		...
		model.put("className", StringUtils
				.uncapitalize(genScheme.getGenTable().getClassName()));
		model.put("ClassName",
				StringUtils.capitalize(genScheme.getGenTable().getClassName()));
		...
		model.put("urlPrefix",
				model.get("moduleName")
						+ (StringUtils.isNotBlank(genScheme.getSubModuleName())
								? "/" + StringUtils
										.lowerCase(genScheme.getSubModuleName())
								: "")
						+ "/" + model.get("className"));
	...
	public static String generateToFile(GenTemplate tpl,
		...
		String content = FreeMarkers
				.renderString(StringUtils.trimToEmpty(tpl.getContent()), model);
		logger.debug(" content === \r\n" + content);

		// 如果选择替换文件，则删除原文件
		if (isReplaceFile && !isProperties(fileName)) {
	...
	public static boolean isProperties(String fileName) {
		return fileName.endsWith("message.properties");
	}
