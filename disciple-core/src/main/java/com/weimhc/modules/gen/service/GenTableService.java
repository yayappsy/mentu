...
import com.weimhc.modules.gen.dao.GenDataBaseDictDao;
...
public class GenTableService extends BaseServiceImpl {
		...
		if (StringUtils.isNotBlank(genTable.getName())) {
			...
			if (list.size() > 0) {
				...
				if (StringUtils.isBlank(genTable.getId())) {
					...
					genTable.setClassName(StringUtils
							.toCapitalizeCamelCase(genTable.getName()));
				}

				// 添加新列
				List<GenTableColumn> columnList = genDataBaseDictDao
