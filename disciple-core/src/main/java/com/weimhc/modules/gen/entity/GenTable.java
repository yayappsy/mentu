 ...
 * 
 */
package com.weimhc.modules.gen.entity;
...
public class GenTable extends DataEntity<GenTable> {
		...
		for (GenTableColumn column : getColumnList()) {
			...
			if (column.getIsNotBaseField()) {
				...
				for (String ann : column.getAnnotationList()) {
					if (!importList.contains(
							StringUtils.substringBeforeLast(ann, "("))) {
						importList
								.add(StringUtils.substringBeforeLast(ann, "("));
					}
				}
			}
		}
	...
	public Boolean getParentExists() {
		return parent != null && StringUtils.isNotBlank(parentTable)
