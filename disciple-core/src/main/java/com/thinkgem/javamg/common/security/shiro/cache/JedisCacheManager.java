	...
	public class JedisCache<K, V> implements Cache<K, V> {
		...
		public V get(K key) throws CacheException {
			...
			try {
				...
				value = (V) JRedisSerializationUtils
						.toObject(jedis.hget(JRedisSerializationUtils.getBytesKey(cacheKeyName),
								JRedisSerializationUtils.getBytesKey(key)));
		...
		public V put(K key, V value) throws CacheException {
			...
			try {
				jedis = JedisUtils.getResource();
				jedis.hset(JRedisSerializationUtils.getBytesKey(cacheKeyName),
						JRedisSerializationUtils.getBytesKey(key),
		...
		public V remove(K key) throws CacheException {
			...
			try {
				...
				value = (V) JRedisSerializationUtils
						.toObject(jedis.hget(JRedisSerializationUtils.getBytesKey(cacheKeyName),
								JRedisSerializationUtils.getBytesKey(key)));
				jedis.hdel(JRedisSerializationUtils.getBytesKey(cacheKeyName),
						JRedisSerializationUtils.getBytesKey(key));
		...
		public int size() {
			...
			try {
				jedis = JedisUtils.getResource();
				size = jedis.hlen(JRedisSerializationUtils.getBytesKey(cacheKeyName)).intValue();
		...
		public Set<K> keys() {
			...
			try {
				jedis = JedisUtils.getResource();
				Set<byte[]> set = jedis.hkeys(JRedisSerializationUtils.getBytesKey(cacheKeyName));
				for (byte[] key : set) {
					keys.add((K) key);
				}
		...
		public Collection<V> values() {
			...
			try {
				jedis = JedisUtils.getResource();
				Collection<byte[]> col = jedis
						.hvals(JRedisSerializationUtils.getBytesKey(cacheKeyName));
				for (byte[] val : col) {
					vals.add((V) val);
				}
