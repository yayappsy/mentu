...
import com.thinkgem.javamg.common.utils.JRedisSerializationUtils;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	public void update(Session session) throws UnknownSessionException {
		...
		try {
			...
			jedis.hset(sessionKeyPrefix, session.getId().toString(), principalId + "|"
					+ session.getTimeout() + "|" + session.getLastAccessTime().getTime());
			jedis.set(JRedisSerializationUtils.getBytesKey(sessionKeyPrefix + session.getId()),
					JRedisSerializationUtils.toBytes(session));
	...
	public void delete(Session session) {
		...
		try {
			...
			jedis.hdel(JRedisSerializationUtils.getBytesKey(sessionKeyPrefix),
					JRedisSerializationUtils.getBytesKey(session.getId().toString()));
			jedis.del(JRedisSerializationUtils.getBytesKey(sessionKeyPrefix + session.getId()));
						...
						try {
							...
							if (principal != null) {
								...
								if (principal.toString()
										.equals(pc != null ? pc.getPrimaryPrincipal().toString()
												: StringUtils.EMPTY)) {
									isActiveSession = true;
								}
							}
	...
	protected Session doReadSession(Serializable sessionId) {
		...
		try {
			...
			session = (Session) JRedisSerializationUtils.toObject(
					jedis.get(JRedisSerializationUtils.getBytesKey(sessionKeyPrefix + sessionId)));
			// }
